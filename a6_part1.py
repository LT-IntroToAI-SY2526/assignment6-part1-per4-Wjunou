...
def create_scatter_plot(data):
    ...
    plt.xlabel('Hours Studied(Hrs)', fontsize=12)
    ...
    print("\n✓ Scatter plot saved as 'scores_scatter.png'")
    plt.show()
...
def visualize_results(X_train, y_train, X_test, y_test, predictions, model):
    ...
    plt.scatter(X_test, y_test, color='green', alpha=0.7, label='Training Data')
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig('test_scores_predictions.png', dpi=300, bbox_inches='tight')
    print("\n✓ Predictions plot saved as 'test_scores_predictions.png'")
    plt.show()
...
def make_prediction(model, hours):
    ...
    print(f"\n=== New Prediction ===")
    print(f"If temperature is {hours}Hrs, predicted scores: {predicted_scores:.2f}%")
